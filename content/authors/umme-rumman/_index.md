---
title: "Umme Rumman"
role: Information Service Assistant; Digital Marketing Executive (SEO)
avatar: /media/people/umme-rumman.jpg
organizations:
  - name: Tottho Apa (Government Project)
    url: https://totthoapa.gov.bd/
  # - name: Radisson Digital Technologies Ltd.
  #   url: https://www.radissonbd.com/
interests:
  - Software quality assurance (manual & automation)
  - Technical troubleshooting and support
  - SEO / SEM / SMM
  - Web content and analytics
  - Go programming (learning)
social:
  - icon: envelope
    icon_pack: fas
    link: mailto:urumman78@gmail.com
  - icon: linkedin
    icon_pack: fab
    link: https://www.linkedin.com/in/umme-rumman-9861ab175/
  - icon: github
    icon_pack: fab
    link: https://github.com/urumman78-art
email: urumman78@gmail.com
bio: "Experienced IT professional with a background in software QA, technical support, and digital marketing. Skilled in troubleshooting systems, coordinating projects, and optimizing search performance; eager to apply these skills in complex, data-driven environments."
highlight_name: false
superuser: false
user_groups:
  - Research Partners
---

Umme Rumman has worked across government and private sectors, combining technical troubleshooting, QA testing, and digital marketing. She supported device, network, and software issues for the Tottho Apa program and drove SEO and campaign reporting at Radisson Digital Technologies. She is expanding into automation testing and Go while continuing to apply analytics and reporting to improve service reliability.
